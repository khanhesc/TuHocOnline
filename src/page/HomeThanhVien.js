...
class HomeThanhVien extends Component {
  ...
  componentWillMount() { 
    ...
    this.setState({
      tocDoNiemPhat: this.app.loginInfo.tocDoNiemPhat == null ? this.app.settings.tocDoNiemPhat.toString() : this.app.loginInfo.tocDoNiemPhat.toString(),      
    ...
    if (result.code == 200) {
      ...
      if (data.success) {
        ...
        this.setState({
          ...
          tocDoNiemPhat: data.entity.tocDoNiemPhat != null ? data.entity.tocDoNiemPhat.toString() : "",
          tgNiemMatChu: data.entity.tgNiemMatChu != null ? data.entity.tgNiemMatChu.toString() : "",
        },()=> {this.diemTichLuy();});            
  ...
  async save() {
    ...
    if (this.state.soLuongDanhHieu.trim() == "") {
      valid = false;
      this.setState({errorSoLuongDanhHieu: "Hãy nhập số lượng bấm lúc Hướng Tâm Niệm"});
    }

    if (this.state.tocDoNiemPhat.trim() == "") {
      valid = false;
      this.setState({errorTocDoNiemPhat: "Hãy nhập tốc độ niệm phật trung bình"});
    }

    if (this.state.soLuongMatChu.trim() == "") {
      valid = false;
      this.setState({errorSoLuongMatChu: "Hãy nhập số lượng bấm lúc Nhiếp Tâm Niệm"});
    }

    if (this.state.tgNiemMatChu.trim() == "") {
      valid = false;
      this.setState({errorTGNiemMatChu: "Hãy nhập khoảng thời gian giữa 2 lần bấm khi nhiếp tâm niệm"});
    }    

    if (this.state.thoiGianRanh == "") {
      valid = false;
      this.setState({errorThoiGianRanh: "Hãy nhập thời gian rảnh"});
    }
          ...
          <View style={[s.thoiGianRanhPopupStyle.container, this.state.orientation == "LANDSCAPE" ? this.thoiGianRanhPopupOriginStyle.landscape.container : this.thoiGianRanhPopupOriginStyle.container, {zIndex: 30}]}>
            <View style={[s.thoiGianRanhPopupStyle.inner, this.state.orientation == "LANDSCAPE" ? this.thoiGianRanhPopupOriginStyle.landscape.inner : this.thoiGianRanhPopupOriginStyle.inner]}>
              <View style={{flexDirection : "row", justifyContent: "space-between", alignItems: "center", height: 50, backgroundColor: Colors.navbarBackgroundColor}}>
                <Text style={{paddingLeft: 10, color: Colors.buttonColor, fontWeight: "bold"}}>Thời gian rảnh</Text>
