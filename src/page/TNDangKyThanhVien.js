...
class TNDangKyThanhVien extends Component {
  constructor(props) {
    super(props);
  ...
  async save() {
    ...
    user.idTruongNhom = this.app.loginInfo.id;

    this.setState({spinner: true});
    let result = await api.thanhVienDangKy(user);
  ...
  onTGNiemMatChuChange(value) {
    var user = this.state.user;
    user.tgNiemMatChu = value;
    this.setState({user: user, errorTGNiemMatChu: null});  
  }  
        ...
        <Container>
          ...
          <Content>
            ...
            <Form>
              ...
              <Item stackedLabel>
                <Label>Khi nhiếp tâm niệm - Thời gian giữa 2 lần bấm (giá trị mặc định chung thường dùng nhất)</Label>
                <Input keyboardType="numeric" maxLength={2} placeholderTextColor="gray" placeholder="bao nhiêu giây?" value={this.state.user.tgNiemMatChu} onChangeText={this.onTGNiemMatChuChange.bind(this)}/>
              </Item>
...
export default TNDangKyThanhVien;