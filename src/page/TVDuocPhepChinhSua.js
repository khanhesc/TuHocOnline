      ...
      <View key="header0" style={[s.table.row, s.table.borderBottom, s.table.borderLeft, s.table.borderRight, s.table.borderTop, {marginTop: 5}]}>
        <View style={[s.table.column, s.table.width75, s.table.borderRight]}>
              ...
              <View style={[s.table.row, s.table.width100, s.table.borderBottom]}>
                <Text style={[s.table.text, s.table.textHeader]}>Tuần (Từ Ngày Đến Ngày)</Text>
              </View>

              <View style={[s.table.row, s.table.width100]}>     
                <View style={[s.table.width25, s.table.borderRight]}>
                  <Text style={[s.table.text, s.table.textHeader]}>SLượng danh hiệu</Text>
                </View>

                <View style={[s.table.width20, s.table.borderRight]}>
                  <Text style={[s.table.text, s.table.textHeader]}>SL Nhiếp Niệm</Text>
                </View>

                <View style={[s.table.width35, s.table.borderRight]}>
                  <Text style={[s.table.text, s.table.textHeader]}>thời gian nghe pháp, công phu</Text>
                </View>

                <View style={[s.table.width20]}>
                  <Text style={[s.table.text, s.table.textHeader]}>Thời Gian rảnh</Text>
                </View>
  ...
  getJSXRow(i, item, chan) {
    ...
    return (
      <View key={"item" + i} style={[s.table.row, s.table.borderBottom, s.table.borderLeft, s.table.borderRight, chan ? styles.chan : styles.le]}>
        <View style={[s.table.column, s.table.width75, s.table.borderRight]}>
              <View style={[s.table.row, s.table.width100, s.table.borderBottom]}>
                <Text style={[s.table.text, s.table.textHeader]}>Tuần {item.week}/{item.month} ({
                  item.fromDate == item.toDate ? 
                  "Ngày " + funcs.getValueFromServerDate(item.fromDate, "day") + "/" + funcs.getValueFromServerDate(item.fromDate, "month")
                  : "Từ Ngày " + funcs.getValueFromServerDate(item.fromDate, "day") + "/" + funcs.getValueFromServerDate(item.fromDate, "month")
              ...
              <View style={[s.table.row, s.table.width100]}>     
                ...
                <View style={[s.table.width20, s.table.borderRight]}>
                  <Text style={[s.table.text]}>{item.soLuongMatChu}</Text>
                </View>

                <View style={[s.table.width35, s.table.borderRight]}>
                  <Text style={[s.table.text]}>{item.tgNghePhap} - {item.tgTungKinh}</Text>
                </View>
