...
class YoutubePlayList extends Component {
  ...
  renderVideoPlayer(video) {
    ...
    this.setState({url: api.getYoutubeEmbedUrl(video.id),videoDefault:video.id,heightVideo:heightVideo});    
  }
  ...
  renderListItem(item, i) {
    ...
    return (
      <ListItem key={"a" + funcs.guid()} thumbnail>
        ...
        <Body>
          <TouchableOpacity onPress={()=>this.onChangeVideo(item, i)}>
          ...
          <View style={[styles.columnCenter, this.state.url != "" ? (this.state.fullScreen ? s.playerFullScreen : {width:width,height:this.state.heightVideo}) : {}]}>
            {
              this.state.url != "" ? (
                <WebView allowsInlineMediaPlayback={true} 
                  ...
                  source={{ uri: "http://tinhdo.vn/videoplayer.html?videoID="+this.state.videoDefault}}
